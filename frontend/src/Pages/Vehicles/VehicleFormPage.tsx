      ...
      <div className="form-card-header flex items-center gap-2">
        {typeof icon === "string" ? (
          <span
            ...
            style={{ fontSize: "20px", color, fontWeight: "normal" }}
          >
            {icon}
          </span>
        ) : (
      ...
      try {
        ...
        if (mode === "edit" && vehicleId) {
          ...
          if (vehicleRes.data.success) {
            ...
            const gd = await tenantApi.get(
              `/gps-device/for/dropdown?current=${data.gps_device_id || ""}`,
            );
            setBeacons(Array.isArray(gd.data) ? gd.data : gd.data?.data || []);

            // Fetch drivers including the currently assigned one
            const driversRes = await tenantApi.get(
              `/drivers/unassigned?current_assigned=${data.assigned_driver_id || ""}`,
            );
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (
          (data[k] as any) instanceof FileList ||
          k.includes("_doc") ||
          k.includes("_certificate") ||
          k.includes("_proof")
        )
          return;
      ...
      const response =
        mode === "create"
          ? await tenantApi.post("/vehicles", formData, {
              headers: { "Content-Type": "multipart/form-data" },
            })
          : await tenantApi.put(`/vehicles/${vehicleId}`, formData, {
              headers: { "Content-Type": "multipart/form-data" },
            });
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  className={`form-input ${errors.vehicle_number ? "border-red-500 bg-red-50" : ""}`}
                  placeholder="e.g. MH12AB1234"
                />
                {errors.vehicle_number && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.vehicle_number.message}</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
                ...
                <select
                  ...
                  className={`form-select ${errors.vehicle_type ? "border-red-500 bg-red-50" : ""}`}
                >
                ...
                {errors.vehicle_type && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.vehicle_type.message}</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
                ...
                <input
                  ...
                  className={`form-input ${errors.manufacturer ? "border-red-500 bg-red-50" : ""}`}
                  placeholder="e.g. Tata, Ashok Leyland"
                />
                {errors.manufacturer && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.manufacturer.message}</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
                ...
                <input
                  ...
                  className={`form-input ${errors.vehicle_model ? "border-red-500 bg-red-50" : ""}`}
                  placeholder="e.g. Eicher 20.15"
                />
                {errors.vehicle_model && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.vehicle_model.message}</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <select {...register("gps_device_id")} className="form-select">
                  ...
                  {gps.map((g: any) => (
                    <option key={g.id} value={g.device_id}>
                      {g.device_id} ({g.sim_number || g.imei_number || ""})
                    </option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input
                  {...register("gps_sim_number")}
                  className="form-input"
                  placeholder="GPS SIM Number"
                />
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
          ...
          <FormSection title="Permit Details" icon="verified" color="#059669">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  {...register("permit_issue_date", { required: true })}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  {...register("permit_expiry_date", { required: true })}
                  className="form-input"
                />
              </div>
            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      {...register("insurance_provider_name", { required: true })}
                      className="form-input"
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      {...register("insurance_expiry_date", { required: true })}
                      className="form-input"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        {...register("fitness_certificate_number", { required: true })}
                        className="form-input"
                        placeholder="RTO Fitness ID"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="date"
                        {...register("fitness_expiry_date", { required: true })}
                        className="form-input"
                      />
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="form-label">Certificate Number*</label>
                      <input
                        {...register("pollution_certificate_number", { required: true })}
                        className="form-input"
                        placeholder="Pollution Cert No."
                      />
                    </div>
                    <div>
                      <label className="form-label">Expiry Date*</label>
                      <input
                        type="date"
                        {...register("pollution_expiry_date", { required: true })}
                        className="form-input"
                      />
                    </div>
                  </div>
          ...
          <FormSection title="Ownership Details" icon="badge" color="#f59e0b">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  {...register("owner_contact_number", { required: true })}
                  className="form-input"
              ...
              <div>
                ...
                <input
                  ...
                  disabled={ownershipType !== "contract"}
                  style={{ opacity: ownershipType === "contract" ? 1 : 0.45 }}
                />
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
              ...
              <div>
                <label className="form-label flex items-center gap-1.5">
                  <Building2 size={14} className="text-slate-400" />
                  Organisation / Fleet Name{" "}
                  <span className="text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded ml-1">
                    OPT
                  </span>
                </label>
                <input
                  {...register("organisation_fleet_name")}
                  className="form-input"
                  placeholder="e.g. ABC School Fleet"
                />
              </div>
            </div>
          </FormSection>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { key: "owner_id_proof", label: "Owner ID Proof*", required: true },
                {
                  ...
                  required: false,
                  note: 'Only required when Ownership Type is "Contract"',
                },
              ].map((doc) => {
                  ...
                  <div key={doc.key} className="flex flex-col">
                    ...
                    <div className="relative group">
                      ...
                      <div className="border-2 border-dashed border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center bg-slate-50 group-hover:bg-white group-hover:border-primary/50 transition-all duration-200 h-[110px] text-center">
                        <span className="material-symbols-outlined text-slate-400 group-hover:text-primary transition-colors duration-200 text-[28px]">
                          cloud_upload
                        </span>
                      ...
                      {existingPath && (
                        <div className="absolute top-2 right-2">
                          <div
                            className="bg-emerald-500 text-white rounded-full p-0.5 shadow-sm"
                            title="File Synced"
                          >
          ...
          <FormSection title="Maintenance" icon="build" color="#64748b">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  {...register("tyre_replacement_due_date")}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  {...register("battery_replacement_due_date")}
                  className="form-input"
                />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="form-input"
                  rows={3}
                  placeholder="Add any additional notes, special conditions or instructions about this vehicle…"
                />
              </div>
            </div>
          </FormSection>
