...
import { Search, Download, Pencil, Trash2, Gavel, BookOpen, FileText } from "lucide-react";
...
const ComplianceIndexPage = () => {
  ...
  const stats = [
    ...
    {
      label: "Compliant",
      value: records.filter((r) => r.status === "Compliant").length.toString(),
      icon: Gavel,
      colorClass: "bg-emerald-50 text-emerald-600",
    },
    {
      label: "Non-Compliant",
      value: records.filter((r) => r.status === "Non-Compliant").length.toString(),
      icon: Gavel,
      colorClass: "bg-rose-50 text-rose-600",
    },
    {
      label: "Pending",
      value: records.filter((r) => r.status === "Pending Review").length.toString(),
      icon: Gavel,
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-10 font-[var(--font-manrope)]">
      ...
      >
        <button
          className="flex items-center gap-2 px-5 py-[11px] bg-white text-[#475569] border border-[#e2e8f0] rounded-[12px] text-[12.5px] font-[800] shadow-sm hover:border-[#7c3aed] hover:text-[#7c3aed] transition-all"
      ...
      <div className="px-6">
        ...
        <div className="bg-white rounded-[18px] border border-[#eef2f6] shadow-[0_2px_12px_rgba(30,41,59,0.03)] p-8 mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-[15px] font-[900] text-[#1e293b] uppercase tracking-wider">
              RTO Guidelines Framework
            </h3>
          </div>
          <p className="text-[13px] font-[600] text-slate-500 mb-6 leading-relaxed">
            Official government guidelines and legal compliance requirements for motor driving
            schools and educational transport services.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              {
                title: "Licensing",
                text: "Schools must maintain valid registration renewed every 5 years.",
                color: "#7c3aed",
              },
              {
                title: "Instructors",
                text: "Min. 5 years experience with valid teaching certificates.",
                color: "#3b82f6",
              },
              {
                title: "Safety Systems",
                text: "Dual control systems mandatory for all training vehicles.",
                color: "#ef4444",
              },
              {
                title: "Signage",
                text: "Vehicles must display prominent 'L' signs at front and rear.",
                color: "#f59e0b",
              },
            ].map((b, i) => (
              <div
                key={i}
                className="p-5 rounded-[14px] bg-[#f8fafc] border-l-[4px]"
            ...
            {loading ? (
              ...
              <div className="py-24">
                <EmptyState
                  ...
                  icon={<Gavel size={64} className="text-slate-200 mb-4" />}
                />
              </div>
            ) : (
              <>
                      ...
                      {records.map((r, i) => (
                        ...
                        >
                          <Td className="py-5 pl-8">
                            <div className="flex items-center gap-4">
                              <div className="text-[11px] font-[900] text-[#7c3aed] bg-[#f5f3ff] px-2 py-0.5 rounded-md border border-[#ddd6fe]">
                                #{r.id}
                              </div>
                              <span className="text-[13.5px] font-[900] text-[#1e293b]">
                                {r.document_name}
                              </span>
                            </div>
                          </Td>
                          <Td className="py-5 text-[13px] font-[700] text-[#475569]">
                            {r.authority_name}
                          </Td>
                          <Td className="py-5 text-center">
                            <span className="px-2.5 py-1 bg-[#f1f5f9] text-[#64748b] text-[10px] font-[900] rounded-[6px] uppercase tracking-wider">
                              {r.category}
                            </span>
                          </Td>
                          <Td className="py-5 text-center">
                            <span
                              className={`px-3 py-1.5 rounded-full text-[10px] font-[900] uppercase tracking-wider border shadow-xs ${
                                r.status === "Compliant"
                          ...
                          <Td className="py-5 text-center pr-8">
                            <div className="flex justify-center gap-1.5">
                              <button
                                onClick={() => setIndividualExport(r)}
                                className="p-2 text-[#64748b] hover:text-indigo-600 hover:bg-indigo-50 rounded-[8px] transition-all"
                              ...
                              <button
                                ...
                                className="p-2 text-[#64748b] hover:text-rose-600 hover:bg-rose-50 rounded-[8px] transition-all"
                                title="Delete"
                              >
                                <Trash2 size={17} />
                              </button>
