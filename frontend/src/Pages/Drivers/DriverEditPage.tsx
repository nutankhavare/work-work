...
const FormSection = ({ title, icon, children, color = "var(--primary)" }: any) => (
  <div className="form-card">
    <div className="form-card-header">
      <span className="material-symbols-outlined ms" style={{ color }}>
        {icon}
      </span>
...
const DriverEditPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          tenantApi.get(
            `/beacon-device/for/dropdown?current=${encodeURIComponent(driver.beacon_id || "")}`,
          ),
        ]);
  ...
  useEffect(() => {
    ...
    tenantApi
      .get(`/masters/forms/dropdowns/districts/${selectedState}`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    if (pinStr.length === 6 && !loading) {
      axios
        .get(`https://api.postalpincode.in/pincode/${pinStr}`)
        .then((res) => {
          if (res.data?.[0]?.Status === "Success") {
            const po = res.data[0].PostOffice[0];
            setValue("city", po.Block || po.District, { shouldValidate: true });
            const matchState =
              dropdowns.states.find((s) => s.state.toLowerCase() === po.State.toLowerCase())
                ?.state || po.State;
        ...
        .catch((err) => {
          console.warn(
            "Postal Pin Code API failed (possibly due to SSL/certificate issues). Gracefully degrading to manual entry.",
            err,
          );
        });
    }
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        ...
        if (data[k] instanceof FileList || k === "license_insurance" || (k as string) === "user")
          return;
      ...
      await tenantApi.put(`/drivers/${id}`, formData, {
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                ...
                >
                  {photoPreview ? (
                    ...
                    <>
                      <span className="material-symbols-outlined text-[28px] text-[#cbd5e1] group-hover:text-[#7c3aed]">
                        add_a_photo
                      </span>
                      <span className="text-[9px] font-[800] text-[#94a3b8] mt-1 group-hover:text-[#7c3aed]">
                        UPDATE
                      </span>
                    </>
                  )}
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`form-input ${errors.employee_id ? "border-red-500 bg-red-50" : ""}`}
                    placeholder="DRV-1001"
                  />
                  {errors.employee_id && (
                    <p className="text-[10px] text-red-500 mt-1">{errors.employee_id.message}</p>
                  )}
                </div>
                <div>
                  <label className="form-label flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    className={`form-input ${errors.first_name ? "border-red-500 bg-red-50" : ""}`}
                    placeholder="John"
                  />
                  {errors.first_name && (
                    <p className="text-[10px] text-red-500 mt-1">{errors.first_name.message}</p>
                  )}
                </div>
                <div>
                  <label className="form-label flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    className={`form-input ${errors.last_name ? "border-red-500 bg-red-50" : ""}`}
                    placeholder="Doe"
                  />
                  {errors.last_name && (
                    <p className="text-[10px] text-red-500 mt-1">{errors.last_name.message}</p>
                  )}
                </div>
                <div>
                  <label className="form-label flex items-center gap-1.5">
                ...
                <div>
                  ...
                  <select {...register("marital_status")} className="form-select">
                    ...
                    {dropdowns.maritalStatuses.map((d) => (
                      <option key={d.value} value={d.value}>
                        {d.value}
                      </option>
                    ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div>
                ...
                <select {...register("employment_type")} className="form-select">
                  {dropdowns.employmentTypes.map((d) => (
                    <option key={d.value} value={d.value}>
                      {d.value}
                    </option>
                  ))}
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={10}
                  pattern="[0-9]{10}"
                  className={`form-input ${errors.mobile_number ? "border-red-500 bg-red-50" : ""}`}
                ...
                {errors.mobile_number && (
                  <p className="text-[10px] text-red-500 mt-1">{errors.mobile_number.message}</p>
                )}
              </div>
              <div>
                <label className="form-label flex items-center gap-1.5">
              ...
              <div>
                ...
                <select {...register("beacon_id")} className="form-select">
                  ...
                  {dropdowns.beacons.map((b) => (
                    <option key={b.id} value={b.device_id}>
                      {b.device_id}
                    </option>
                  ))}
                </select>
          ...
          <FormSection title="Permits & Licenses" icon="id_card">
            <div className="space-y-4">
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 bg-[#fafbff] rounded-[10px] border border-[#f1f5f9] relative"
                >
                  <div className="md:col-span-1">
                    <label className="form-label">Class</label>
                    <select
                      {...register(`license_insurance.${index}.type` as any)}
                      className="form-select"
                    >
                      {dropdowns.fileTypes.map((d) => (
                        <option key={d.value} value={d.value}>
                          {d.value}
                        </option>
                      ))}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      {...register(`license_insurance.${index}.number` as any)}
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      {...register(`license_insurance.${index}.exp_date` as any)}
                      className="form-input"
                    />
                  </div>
                  <div className="flex items-end">
                    <button
                      ...
                      className="p-3 bg-white text-red-500 border border-red-100 rounded-[8px] hover:bg-red-50"
                    >
              ...
              <button
                ...
                className="w-full py-3 border-2 border-dashed border-[#e2e8f0] rounded-[10px] text-[#64748b] text-[11px] font-[900] hover:border-[#7c3aed] hover:text-[#7c3aed] transition-all uppercase tracking-widest"
              >
                + Add Permit
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div className="form-card !mb-0">
              <div className="form-card-header !bg-[#f0f9ff]/50">
                <span className="material-symbols-outlined ms" style={{ color: "#0ea5e9" }}>
                  emergency
                </span>
                <span className="text-[11px] font-[900] tracking-[0.07em] uppercase text-[#1e293b]">
                  Primary Contact
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      {...register("primary_person_phone_1")}
                      type="tel"
                      maxLength={10}
                      pattern="[0-9]{10}"
                      className="form-input"
            ...
            <div className="form-card !mb-0">
              <div className="form-card-header !bg-[#f1f5f9]/50">
                <span className="material-symbols-outlined ms" style={{ color: "#64748b" }}>
                  contact_support
                </span>
                <span className="text-[11px] font-[900] tracking-[0.07em] uppercase text-[#1e293b]">
                  Secondary Contact
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="form-label">Phone</label>
                    <input
                      {...register("secondary_person_phone_1")}
                      type="tel"
                      maxLength={10}
                      pattern="[0-9]{10}"
                      className="form-input"
                  ...
                  <div>
                    ...
                    <input
                      type="email"
                      {...register("secondary_person_email")}
                      className="form-input"
                    />
                  </div>
                </div>
            ...
            {[
              ...
              return (
                <div key={docKey}>
                  ...
                  <input
                    type="file"
                    {...register(docKey as any)}
                    className="form-input h-[45px] file:hidden pt-[11px] cursor-pointer"
                  />
                  {existingPath && (
                    <p className="text-[9px] text-slate-400 mt-1 truncate max-w-[200px]">
                      Existing: {existingPath.split("/").pop()}
                    </p>
                  )}
                </div>
              );
