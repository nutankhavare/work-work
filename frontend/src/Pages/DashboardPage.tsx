...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setVehicles([
          { vehicle_number: "VAN-001", latitude: 12.9716, longitude: 77.5946, status: 'moving' },
          { vehicle_number: "VAN-002", latitude: 12.9816, longitude: 77.6046, status: 'idle' }
        ] as any);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-[var(--font-manrope)]">
      ...
      {/* ── Map Section ── */}
      <div className="flex-1 bg-white rounded-[18px] border border-[#eef2f6] shadow-[0_2px_12px_rgba(30,41,59,0.03)] overflow-hidden relative min-h-[500px]">
        {loading ? (
